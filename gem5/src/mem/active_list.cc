...
namespace gem5{
    ...
    ActiveList::recvTimingReq(PacketPtr pkt)
    {
        ...
        count_reqs++;
        if(count_reqs%4000 == 0){
            DPRINTF(OneThousand, "%s: isWrite(): %d, Active List size = %d, num_reqs = %ld\n", __func__,  pkt->isWrite(), queueLength(), count_reqs);
        }
    ...
    ActiveList::processNextReadEvent()
    {
        ...
        if (queueLength() < 1){
            ...
            DPRINTF(ActiveList, "%s: Read Value: %d, queueLength: %d, sender id: %d\n", __func__, valueMap.front(), queueLength(), pkt->req->requestorId());
            // DPRINTF(ActiveList, "%s: Read Value:, queueLength: %d\n", __func__,  queueLength());

            pkt->setLE(valueMap.front());
            valueMap.pop_front();
            // below comment block allows 2 different requestors to read from the queue
            // uint64_t curr_requestor = pkt->req->requestorId();
            // if(curr_requestor == requestor_1){
            ...
            //     if(alternate_r == 1){
            //         requestor_1 = requestor_2;
            //         alternate_r = alternate_r2;
            //         requestor_2 = 99999990;
            //         alternate_r2 = 1;
            //     }
            // }
            // else if(curr_requestor == requestor_2){
            //     pkt->setLE(valueMap[alternate_r2]);
            //     valueMap.erase(valueMap.begin()+alternate_r2);// error here // get<0> is temporary
            //     alternate_r2 = alternate_r2 ^ 1;
            //     if(alternate_r2 == 1){
            //         requestor_2 = 99999990;
            //         alternate_r2 = 1;
            //     }
            // }
            // else if(requestor_1 == 99999990){
            //     requestor_1 = curr_requestor;
            //     pkt->setLE(valueMap[alternate_r]);
            //     valueMap.erase(valueMap.begin()+alternate_r);// error here // get<0> is temporary
            //     alternate_r = alternate_r ^ 1;
            // }
            // else if(requestor_2 == 99999990){
            //     requestor_2 = curr_requestor;
            //     pkt->setLE(valueMap[alternate_r2]);
            //     valueMap.erase(valueMap.begin()+alternate_r2);// error here // get<0> is temporary
            //     alternate_r2 = alternate_r2 ^ 1;
            // }
            // else{
            //     panic("requestor id not recognized");
            // }
