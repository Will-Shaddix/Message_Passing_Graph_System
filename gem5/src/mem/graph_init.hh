...
class GraphInit : public ClockedObject
{
  ...
  public:
    ...
    int num_partitions;
    uint32_t size_of_partition; // 151487 for liveJournal 32 partitions.
    uint8_t edge_partitioned; // 1 if edge partitioned, 0 if edge not partitioned
    uint8_t vertex_partitioned; // 1 if vertex partitioned, 0 if vertex not partitioned
    uint64_t mem_ctrl_size; // 256 MiB
