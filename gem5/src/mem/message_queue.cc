...
namespace gem5{
    ...
    MessageQueue::recvTimingReq(PacketPtr pkt)
    {
        ...
        count_reqs++;
        if(count_reqs%20000 == 0){
            DPRINTF(OneThousand, "%s: isWrite(): %d, Message Queue size = %d, num_reqs = %ld\n", __func__,  pkt->isWrite(), queueLength(), count_reqs);
        }
    ...
    MessageQueue::processNextWriteEvent()
    {
        ...
        if(queueLength() >= queueSize){
            ...
            // below if is for this to write to memory that its done
            //  if(queueLength() == 0){
