...
num_vertex_mem_ctrls = int(num_vertex_gib*2) # assumes 1 GiB per memory controller
...
binary3 = "configs/William/graph_src/batching/generator_al"
...
for i in range(1, int(num_cores/2) + 1):
    process[(2*i) - 1].cmd = [binary2, str(i - 1), str(int(num_cores/2))] # consumer
    process[(2*i) - 1].pid = 101 + i
    process[2*i].cmd = [binary3, str(i - 1), str(int(num_cores/2))] # generator
